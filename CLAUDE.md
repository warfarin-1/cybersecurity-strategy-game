...
interface View {
    type: ViewType;
    chapter?: ChapterLevel;   // ChapterLevel = 2 | 3 | 4
    stageId?: string;
}
```

`chapter` 和 `stageId` 为可选字段；在 `view.type === "stage"` / `"chapter"` 分支内使用 `!` 断言访问。
...
- `loadControls(lang?)` — 加载 controls CSV（英文或双语版本），使用 papaparse 解析
- `loadThreats(level, lang?)` — 加载对应 Level 的 threats CSV，使用 papaparse 解析
- `loadLevel4Tree(lang?)` — 加载 L4 威胁树 JSON（直接 `res.json()`，无 CSV 解析）
...
`App.css` 使用 **CSS 自定义属性（变量）** 实现双主题；`index.css` 为补充样式。`index.html` 引入 **Rajdhani**（日系标题字体，Google Fonts）与 Inter 两套字体。

### 主题变量体系

`:root`（深色，深靛蓝色调：`#0a0e1a` 系列 + `#00d4ff` 青色强调）和 `[data-theme="light"]`（冷调靛蓝浅色：`#c8d0e8` 系列）块定义全部颜色变量：
...
**日系视觉风格新增 CSS（2026-04-07）：**

| 类名 | 用途 |
|------|------|
| `.chapter-card::before/::after` | 章节卡片四角装饰（12×12 px L形边框，默认青色，completed/tutorial 绿色） |
| `.stage-card::before/::after` | Stage 卡片四角装饰（8×8 px，1px 青色） |
| `.sidebar-title::before/::after` | 侧栏标题 `[ … ]` 括号装饰，青色 |
| `.control-center-header::before/::after` | 控制室标题 `[ … ]` 括号装饰，橙色 |
| `.box-title::before` | 信息盒标题 `// ` 前缀，青色 60% 透明 |
| `[data-theme="light"] .chapter-card` 等 | 浅色模式下六类容器统一加 `box-shadow: 0 2px 8px rgba(0,85,187,0.12)` |
...
**代码自然化（2026-04-07）**
- [x] **papaparse 替换手写 CSV 解析器**：`dataLoader.ts` 中删除 `parseCsvLine`，改用 `Papa.parse<string[]>(raw, { skipEmptyLines: true })`
- [x] **TypeScript 类型注解简化**：去掉 `STAGES_BY_CHAPTER` 的 `Record<>` 注解、`isChapterUnlocked/isStageUnlocked` 的 `: boolean` 返回类型、`Object.entries` 类型断言改为 `Object.values`；`StageMeta` 接口随之删除
- [x] **函数式链式调用 → for 循环**：`unresolvedHigh`（filter）和 `knownThreatTypes`（map+filter）两处改为 `for...of`
- [x] **嵌套三元 → if/else**：`statusLabel`、`statusColor`（4路链式）和控制按钮 `label`（2层嵌套）三处改写
- [x] **View 类型简化**：discriminated union 改为带可选字段的 `interface View { type: ViewType; chapter?: ChapterLevel; stageId?: string }`，约 12 处访问加 `!` 断言

**日系视觉主题（2026-04-07）**
- [x] **Rajdhani 字体**：`index.html` 引入；10 类标题元素统一 `font-family: 'Rajdhani'`，`top-bar-title` 和 `btn-outline` 另加 `text-transform: uppercase`
- [x] **深靛蓝配色**：`:root` 改为 `#0a0e1a` 系深靛蓝 + `#00d4ff` 青色强调；浅色模式改为 `#c8d0e8` 系冷调靛蓝
- [x] **有棱角风格**：主要容器 `border-radius` 全面压缩（卡片 18→4px，pill 999→2px，弹窗 16→6px 等）
- [x] **CSS 伪元素角标**：`.chapter-card` 和 `.stage-card` 用 `::before/::after` 绘制 L 形角标装饰
- [x] **标题方括号装饰**：`.sidebar-title`、`.control-center-header`、`.box-title` 用 `::before/::after content` 加入 `[ ]` / `//` 装饰
- [x] **TopBar/BottomBar 青色边框**：`border-color` 改为 `var(--accent-blue)` + `box-shadow` 发光效果
- [x] **扫描线背景**：`.map-container` 加 `repeating-linear-gradient` 微妙横向扫描线（深色 `rgba(0,212,255,0.015)`，浅色 `rgba(0,102,204,0.02)`）
- [x] **威胁节点左侧色条**：`.threat-node` 改为 `border-left: 3px solid` 配合 mitigated/unresolved 绿红颜色
- [x] **已部署控制高亮**：`.control-deployed-row .sidebar-pill` 加 `border-left: 3px solid var(--accent-green)`
- [x] **浅色模式卡片阴影**：6 类容器在 `[data-theme="light"]` 下统一加 `box-shadow: 0 2px 8px rgba(0,85,187,0.12)`
