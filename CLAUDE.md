...
- 游戏模式切换：**Beginner / Expert**（持久化 localStorage）
- 语言切换：**English / 中文**（持久化 localStorage）
...
### 5.5 扣分逻辑（handleNextTurn）

每回合对未缓解威胁扣分：High×10 / Medium×3 / Low×1

威胁缓解判定：`threat.recommendedControlIds.some(id => deployedControlIds.includes(id))`

### 5.6 通关判定

**L2/L3**：`stageConfig.requiredControlIds` 全部部署后标记 `"completed"`

**L4**：`level4Scenario.subThreatIds` 中每个子威胁的 `recommendedControlIds` 至少有一个已部署（`allSubThreatsMitigated`），则 Scenario 被"阻断"，Stage 完成
...
所有样式均在 `App.css`（深色主题），`index.css` 为补充。
...
- [x] L2/L3 通关检测：所有 `requiredControlIds` 部署后 Stage 标记为 `"completed"`
- [x] L4 威胁树通关逻辑：所有 `subThreatIds` 对应威胁的推荐控制至少部署一个，Scenario 阻断，Stage 完成
...
- [x] `dataLoading` 保护：数据加载期间禁用 Next Turn 按钮；key 为 `${stageId}:${language}`（语言切换触发重新加载）
...
- [x] 模式选择器在地图视图，持久化 localStorage

**双语支持（EN/ZH）**
- [x] 语言切换器在地图视图（English / 中文），持久化 localStorage
...
- [x] `OrgProfile`：三个 Level 对应三家虚构组织（L2: Meridian 零售集团 / L3: Eastbridge 综合医院 / L4: 国家电网运营中心），含双语名称、行业类型、简介标语
- [x] `PromotionEvent`：解锁 L3/L4 时触发晋升弹窗，含经理台词、时间跳跃文本、新职称（双语）；已读状态通过 `localStorage` (`seenPromotion_${level}`) 持久化
- [x] `PLAYER_TITLES` + `getPlayerTitle()`：按已完成章节数推算玩家职称（初级/安全/高级安全顾问），显示于地图视图 TopBar（"职称 · Sentinel Advisory"）
- [x] **Stage 任务简报（Mission Brief）**：每个 Stage 均配有叙事背景简报（`briefing` / `briefingZh`），显示于 Stage 视图顶部，可折叠展开（默认展开）
- [x] **回合反馈消息（feedbackMsg）**：`handleNextTurn` 根据部署情况生成提示文本，通过 `feedbackMsg` prop 传入 BottomBar 显示
...
- [x] 重置后 Stage 状态归零（budget/deployedControlIds/isCompleted/status/turn/logs）
...
- [x] BottomBar：Turn / Budget / Score / 通关状态实时显示，`isLoading` 禁用保护，支持 `language` prop
...
1. **单一主文件**：`App.tsx` 包含所有导航逻辑、游戏状态管理、GlossaryPanel 组件，约 1030 行。如需拆分，可将 GlossaryPanel、Stage 视图、Chapter 视图各自提取为独立组件。
...
1. 确保 L2/L3/L4 全流程无卡死（budget 充足、required controls 可全部部署）
2. 验证扣分力度：连续多回合不部署控制，分数从 100 降至多少
3. 进行正式用户测试，收集反馈

### 阶段 B：体验完善（低优先级）

4. **L2/L3 stage-main-board 可视化升级**：当前为 Threat Status 列表，可升级为资产-威胁矩阵或简易网络拓扑图
5. **chapterState 持久化**（可选）：将 `stageStates` 序列化写入 `localStorage`，使 Stage 内进度跨刷新保留
6. 清理遗留原型文件（`Layout.tsx` 等）
7. 拆分 `App.tsx`：将 GlossaryPanel、各视图组件提取为独立文件
...
| 叙事数据（组织、晋升、职称） | `src/data/narrative.ts` |
